...
export * from './decision-intelligence' // Decision intelligence — Ch 5.1-5.14: signal generation → strategy intelligence → portfolio construction → risk management → position sizing → order decision → execution optimization → smart order routing → broker gateway → exchange execution → post-trade reconciliation → portfolio accounting → PnL & performance attribution → risk analytics & exposure: Canonical Risk Contract, 15-stage pipeline, exposure (§6), VaR/ES (§7, Rule 9), stress (§8)/scenario (§9, Rule 8), liquidity (§11, Rule 10), concentration (§12, Rule 11), regime detection (§10A, Rule 21/22), enterprise netting (§11A, Rule 25/26/27), dual-speed risk states (§15A, Rule 23/24), versioning, governance
