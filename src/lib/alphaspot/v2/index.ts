...
export * from './decision-intelligence' // Decision intelligence — Ch 5.1-5.15: signal generation → strategy intelligence → portfolio construction → risk management → position sizing → order decision → execution optimization → smart order routing → broker gateway → exchange execution → post-trade reconciliation → portfolio accounting → PnL & performance attribution → risk analytics & exposure → compliance & regulatory control: Canonical Compliance Contract, dual pipeline (pre-trade 11 stages + post-trade 11 stages), Compliance DSL (§7A, Rule 8/9), decisions (§6A: APPROVED/WARNING/HARD_VETO + PASSIVE_BREACH/MANDATORY_REVIEW/ESCALATION/EMERGENCY_HALT), mandates (§8, Rule 11), client restrictions (§9, Rule 10), surveillance (§10, Rule 18), AML/KYC (§11, Rule 12), fail-closed (§17), versioning, governance
