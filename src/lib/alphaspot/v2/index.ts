...
export * from './decision-intelligence' // Decision intelligence — Ch 5.1-5.13: signal generation → strategy intelligence → portfolio construction → risk management → position sizing → order decision → execution optimization → smart order routing → broker gateway → exchange execution → post-trade reconciliation → portfolio accounting → PnL & performance attribution: Canonical Performance Contract, 16-stage pipeline, PnL calculation (§7, Rule 8), returns (§8), attribution (§9/9A, Rule 23), valuation state management (§10A, Rule 21/22), benchmark (§10, Rule 9), risk-adjusted metrics (§11, Rule 10), performance restatement (§17A, Rule 24), versioning, governance
